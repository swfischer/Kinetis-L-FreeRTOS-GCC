...
static void usbCoreIrqHandler(uint32_t unused);
...
//   SIM_SCGC4 |= SIM_SCGC4_USBOTG_MASK; // USB Clock Gating
   irqRegister(INT_USB0, usbCoreIrqHandler, 0);
   irqEnable(INT_USB0);
...
void USB_StdReq_Handler(void)
{
   ...
   case mGET_DESC:
      ...
      {
      case mDEVICE:
         EP_IN_Transfer(EP0, (uint8_t*)&usbCfgDevDesc, sizeof(usbCfgDevDesc));
         break;
      case mCONFIGURATION:
         EP_IN_Transfer(EP0, (uint8_t*)&usbCfg, sizeof(usbCfg));
         break;
      case mSTRING:
         EP_IN_Transfer(EP0, (uint8_t*)sStringTable[Setup_Pkt->wValue_l], sStringTable[Setup_Pkt->wValue_l][0]);
         break;
      default:
         USB_EP0_Stall();
         break;
   ...
   case mGET_STATUS:
      status = 0;
      EP_IN_Transfer(EP0, (uint8*)&status, 2);
      break;
   default:
      USB_EP0_Stall();
      break;
